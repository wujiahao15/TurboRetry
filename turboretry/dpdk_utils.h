...
struct application_port_config {
    ...
    uint16_t isolated_mode : 1; /* Set on init to 0 for no isolation, isolated
                                   mode otherwise */
...
/*
 * Initialize a shadow of a DPDK memory pool the shadow will have all of DPDK's
 * memory registered to a device
 *
...
/*
 * Find the DOCA mmap instance that contains the requested address range then
 * allocate DOCA buffer from it
 *
 * @mempool_shadow [in]: shadow of a DPDK memory pool
 * @inventory [in]: a DOCA buffer inventory used for allocating the buffer
 * @mem_range_start [in]: start address of memory range, must be within range of
 * some 'rte_mbuf'
 * @mem_range_size [in]: the size of the memory range in bytes, must be within
