...
struct dpdk_mempool_shadow {
    ...
    struct doca_mmap *
        *mmap_arr;     /* DOCA mmap array that has mapped the packet buffers */
...
static doca_error_t bind_hairpin_queues(uint16_t port_id) {
    ...
    peer_ports_len = rte_eth_hairpin_get_peer_ports(port_id, peer_ports,
                                                    RTE_MAX_ETHPORTS, 1);
    if (peer_ports_len < 0) {
        DOCA_LOG_ERR("Failed to get hairpin peer Rx ports of port %d, (%d)",
...
static doca_error_t setup_hairpin_queues(uint16_t port_id,
    ...
    for (hairpin_q = 0; hairpin_q < hairpin_queue_len; hairpin_q++) {
        ...
        result = rte_eth_tx_hairpin_queue_setup(
            port_id, reserved_hairpin_q_list[hairpin_q], nb_tx_rx_desc,
            &hairpin_conf);
        if (result < 0) {
        ...
        // RX
        hairpin_conf.peers[0].queue = reserved_hairpin_q_list[hairpin_q];
        result = rte_eth_rx_hairpin_queue_setup(
            port_id, reserved_hairpin_q_list[hairpin_q], nb_tx_rx_desc,
            &hairpin_conf);
        if (result < 0) {
            DOCA_LOG_ERR("Failed to setup hairpin queues (%d)", result);
...
static doca_error_t allocate_mempool(const uint32_t total_nb_mbufs,
    ...
    *mbuf_pool =
        rte_pktmbuf_pool_create("MBUF_POOL", total_nb_mbufs, MBUF_CACHE_SIZE, 0,
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t port_init(struct rte_mempool *mbuf_pool, uint8_t port,
    ...
    if (ret < 0) {
        DOCA_LOG_ERR("Failed getting device (port %u) info, error=%s", port,
                     strerror(-ret));
        return DOCA_ERROR_DRIVER;
    }
    ...
    if (nb_hairpin_queues) {
        ...
        if (app_config->port_config.self_hairpin &&
            ...
            result = setup_hairpin_queues(port, port, rss_queue_list,
                                          nb_hairpin_queues / 2);
            if (result != DOCA_SUCCESS) {
            ...
            result = setup_hairpin_queues(port, port ^ 1, rss_queue_list,
                                          nb_hairpin_queues / 2);
            if (result != DOCA_SUCCESS) {
            ...
            if (rte_eth_dev_is_valid_port(port ^ 1))
                ...
                result = setup_hairpin_queues(port, port, rss_queue_list,
                                              nb_hairpin_queues);
            if (result != DOCA_SUCCESS) {
                DOCA_LOG_ERR("Cannot hairpin port %" PRIu8 ", ret=%d", port,
                             result);
                return result;
            }
        }
    ...
    if (ret < 0) {
        DOCA_LOG_ERR("Port %u could not be set isolated mode to %s (%s)", port,
                     isolated ? "true" : "false", error.message);
        return DOCA_ERROR_DRIVER;
    }
    if (isolated)
 ...
 */
static void dpdk_ports_fini(struct application_dpdk_config *app_dpdk_config,
                            uint16_t nb_ports) {
    int result;
    int port_id;
    ...
    for (port_id = nb_ports; port_id >= 0; port_id--) {
        ...
        if (result != 0)
            DOCA_LOG_ERR("rte_eth_dev_stop(): err=%d, port=%u", result,
                         port_id);

        result = rte_eth_dev_close(port_id);
        if (result != 0)
            DOCA_LOG_ERR("rte_eth_dev_close(): err=%d, port=%u", result,
                         port_id);
    }
...
static void print_ether_addr(const struct rte_ether_addr *dmac,
    ...
    DOCA_LOG_DBG("DMAC=%s, SMAC=%s, ether_type=0x%04x", dmac_buf, smac_buf,
                 ethertype);
}
...
static void print_l2_header(const struct rte_mbuf *packet) {
    ...
    print_ether_addr(&eth_hdr->dst_addr, &eth_hdr->src_addr,
 ...
 */
static void print_ipv4_addr(const rte_be32_t dip, const rte_be32_t sip,
                            const char *packet_type) {
    DOCA_LOG_DBG("DIP=%d.%d.%d.%d, SIP=%d.%d.%d.%d, %s",
                 (dip & 0xff000000) >> 24, (dip & 0x00ff0000) >> 16,
                 (dip & 0x0000ff00) >> 8, (dip & 0x000000ff),
                 (sip & 0xff000000) >> 24, (sip & 0x00ff0000) >> 16,
                 (sip & 0x0000ff00) >> 8, (sip & 0x000000ff), packet_type);
}
 ...
 * @packet_type [in]: packet type
 */
static void print_ipv6_addr(const uint8_t dst_addr[16],
...
static void print_l3_header(const struct rte_mbuf *packet) {
    if (RTE_ETH_IS_IPV4_HDR(packet->packet_type)) {
        struct rte_ipv4_hdr *ipv4_hdr = rte_pktmbuf_mtod_offset(
            packet, struct rte_ipv4_hdr *, sizeof(struct rte_ether_hdr));
        ...
        struct rte_ipv6_hdr *ipv6_hdr = rte_pktmbuf_mtod_offset(
            packet, struct rte_ipv6_hdr *, sizeof(struct rte_ether_hdr));
...
static void print_l4_header(const struct rte_mbuf *packet) {
    ...
    ipv4_hdr = rte_pktmbuf_mtod_offset(packet, struct rte_ipv4_hdr *,
 ...
 */
static void dpdk_chunk_to_mmap_cb(struct rte_mempool *mp, void *opaque,
    ...
    if (result != DOCA_SUCCESS) {
        DOCA_LOG_ERR("Unable to set memory range of memory map (input): %s",
    ...
    if (result != DOCA_SUCCESS) {
        DOCA_LOG_ERR("Unable to start memory map: %s",
                     doca_error_get_descr(result));
        doca_mmap_destroy(new_mmap);
        return;
    }
...
dpdk_mempool_shadow_create(struct rte_mempool *mbuf_pool,
    ...
    mempool_shadow->mmap_arr = (struct doca_mmap **)rte_zmalloc(
        NULL, sizeof(struct doca_mmap *) * nb_chunks, 0);
    ...
    nb_iterated_chunks =
        rte_mempool_mem_iter(mbuf_pool, dpdk_chunk_to_mmap_cb, mempool_shadow);
 ...
 */
static doca_error_t
dpdk_ext_chunk_to_mmap(struct dpdk_mempool_shadow *mempool_shadow, void *chunk,
                       size_t len) {
    struct doca_mmap *new_mmap;
    doca_error_t result;
...
struct dpdk_mempool_shadow *
dpdk_mempool_shadow_create_extbuf(const struct rte_pktmbuf_extmem **ext_mem,
    ...
    mempool_shadow->mmap_arr = (struct doca_mmap **)rte_zmalloc(
        NULL, sizeof(struct doca_mmap *) * ext_num, 0);
    if (mempool_shadow->mmap_arr == NULL) {
        DOCA_LOG_ERR("Dynamic allocation failed");
        dpdk_mempool_shadow_destroy(mempool_shadow);
        return NULL;
    ...
    /* Register each chunk in the mempool to an mmap */
    for (i = 0; i < ext_num; ++i) {
        result = dpdk_ext_chunk_to_mmap(mempool_shadow, ext_mem[i]->buf_ptr,
                                        ext_mem[i]->buf_len);
        if (result != DOCA_SUCCESS) {
            dpdk_mempool_shadow_destroy(mempool_shadow);
            return NULL;
...
doca_error_t dpdk_mempool_shadow_find_buf_by_data(
    struct dpdk_mempool_shadow *mempool_shadow,
    struct doca_buf_inventory *inventory, uintptr_t mem_range_start,
    size_t mem_range_size, struct doca_buf **out_buf) {
    ...
    for (mmap_idx = 0; mmap_idx < mempool_shadow->nb_mmaps; mmap_idx++) {
        ...
        /* Following checks that memory range is within the mmap while avoiding
         * integer overflow */
        if (mmap_begin <= mem_range_start &&
            mem_range_start < mmap_begin + mmap_len &&
            mem_range_size <= mmap_begin + mmap_len - mem_range_start)
            return doca_buf_inventory_buf_get_by_data(inventory, mmap,
                                                      (void *)mem_range_start,
                                                      mem_range_size, out_buf);
    }

    return DOCA_ERROR_NOT_FOUND;
}

void print_header_info(const struct rte_mbuf *packet, const bool l2,
                       const bool l3, const bool l4) {
    if (l2)
