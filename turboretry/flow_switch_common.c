...
#define FLOW_SWITCH_DEV_ARGS                                                   \
    "dv_flow_en=2,fdb_def_rule_en=0,vport_match=1,repr_matching_en=0,dv_"      \
    "xmeta_en=4"
#define FLOW_SWITCH_REP_ARG ",representor="

DOCA_LOG_REGISTER(FLOW_SIWTCH_COMMON);
...
doca_error_t register_doca_flow_switch_param(void) {
    ...
    if (result != DOCA_SUCCESS) {
        DOCA_LOG_ERR("Failed to create flow swtich ARGP param: %s",
    ...
    if (result != DOCA_SUCCESS) {
        DOCA_LOG_ERR("Failed to register flow swtich ARGP param: %s",
                     doca_error_get_descr(result));
        return result;
    }
...
doca_error_t init_doca_flow_switch_common(struct flow_switch_ctx *ctx) {
    ...
    for (i = 0; i < ctx->nb_ports; i++) {
        ...
        result =
            open_doca_device_with_pci(ctx->dev_arg[i], NULL, &ctx->doca_dev[i]);
        if (result != DOCA_SUCCESS) {
        ...
        if (ctx->rep_arg[i]) {
            port_args[i] = calloc(1, strlen(FLOW_SWITCH_DEV_ARGS) +
                                         strlen(FLOW_SWITCH_REP_ARG) +
                                         strlen(ctx->rep_arg[i]) + 1);
            if (!port_args[i]) {
                DOCA_LOG_ERR("Failed to allcoate dpdk args port: %d", i);
        ...
        if (result != DOCA_SUCCESS) {
            DOCA_LOG_ERR("Failed to probe DOCA device: %s",
                         doca_error_get_descr(result));
            goto quit;
        }
    }
